...
julia> plot(x->x*exp(-x),0,10)

# with title, label, x-axis and y-axis labels, different color
julia> plot(x->x*exp(-x),0,10,title="Plot-Example", label="x*exp(-x)", xlabel="x", ylabel="y", color=:red)

# save to PDF file
julia> savefig("example_plot.pdf")
```
`plot` returns a handle to the plot that can be used in savefig.
```julia
julia> p = plot(x->x*exp(-x),0,10)

julia> savefig(p, "example_plot.pdf")
```
...
```julia
julia> plot(0:1:10,x->x*exp(-x),label="with coarse resolution", linewidth=2)

julia> plot!(0:0.01:10,x->x*exp(-x),label="with fine resolution", linewidth=2)
```
For the moment, the last thing to present is the error plot.
```julia
julia> x = collect(0:1:10)                             # x values

julia> f(x) = x*exp(-x)                                # f = x -> x*exp(-x)

julia> yerr = 0.1 .* f.(x)                             # y-error (10% of function value) 

julia> y = f.(x) .+ yerr .* randn(length(yerr))        # y values with small errors

julia> plot(x,y,yerror=yerr, st=:scatter)              # scatter plot with error bars
```
