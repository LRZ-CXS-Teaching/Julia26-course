...
# COLUMN-MAJOR: m innermost -> walks memory sequentially -> cache friendly
function matmul_columnmajor!(C, A, B)
    #@assert size(A,2) == size(B,1) "A and B must be compatible!"
    M, K = size(A)
    _, N = size(B)
    @inbounds for n in 1:N, k in 1:K, m in 1:M
        C[m, n] += A[m, k] * B[k, n]
    end
    return nothing
end

# ROW-MAJOR: n innermost -> jumps across columns -> cache unfriendly in Julia
function matmul_rowmajor!(C, A, B)
    M, K = size(A)
    _, N = size(B)
    @inbounds for m in 1:M, k in 1:K, n in 1:N
        C[m, n] += A[m, k] * B[k, n]
    end
    return nothing
end

# Julia has the built-in "mul!" and "*" functions -> dispatches to OpenBLAS/MKL automatically
