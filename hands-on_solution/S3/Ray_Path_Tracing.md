...
julia> render()                        # runs render() code once, possibly compiles things

julia> @profile render()               # takes a profile of render()
...
Having identified some relevant temporaries and type-instabilities, `StaticArrays` where used. And then, also thread parallelism introduced.
<details>
    <summary>raytracer_staticarray.jl</summary>
...
const Vec3 = SVector{3, Float64}                                                    # static size array now
...
function random_in_unit_sphere()
  while true
    p = @SVector [rand()*2-1, rand()*2-1, rand()*2-1]
    if dot(p, p) < 1.0 return p end
  end
end
...
# --- Illumination & Tonemapping ---
function aces_tonemap(x)                                                            # x type inferred now
  a, b, c, d, e = 2.51, 0.03, 2.43, 0.59, 0.14
  @. clamp((x * (a * x + b)) / (x * (c * x + d) + e), 0.0, 1.0)
end
...
function trace_path(ray::Ray, scene, depth::Int, include_emission::Bool=true)
  if depth <= 0 return Vec3(0.0, 0.0, 0.0) end                                       # type stable null
...
# --- szene & rendering ---
function render()
  w, h = 1280, 900                     # window width and height (400,300 for small test)
  samples = 1024                       # samples per pixel; higher for better quality (less noise) (50 for small test)
  exposure = 1.5                       # lighter image > 1
  depth = 12                           # number of bounces (5 for small test)
  scale = 0.73                         # zoom factor
  ...
  @showprogress Threads.@threads for x in 1:w
    for y in 1:h
      col = [0.0, 0.0, 0.0]
      for s in 1:samples
        u = -((x + rand() - w/2) / h ) * scale            # x pixel position
...
These are the demanding render settings - high resolution, large MC sample size, ray render depth - as we used before for the 35  minuts on 80 threads test case.
It eecutes now serially (!! without threads) in 18 minutes. And on 80 threads in 
